...
(First and second paragraphs as they are)

However, properties that enable a species to interact with a broad range of partners (\textit{i.e.}, generalized, high degree species), may switch partners depending on community context. The adaptive foraging theory predicts that there is a trade-off between foraging efficiency and resource availability; thus, if competing species are present, it is expected that there will be a decrease in niche overlap among species present in a given area. In this sense, when other, competing species are present, generalist species can explore different resources, switching their interaction partners. This alternative assembly mechanism can be described as an opportunistic attachment process. In the opportunistic attachment process generalist species, instead of being part of a persistent species' core, are actually the species that explore, opportunistically, the most available resources, that is, the resources that are being use the less. Thus, when you consider the assembly process in a time series, the most persistent species are also the ones that, through time, exhibit the highest degree, but are also the most variable in their network position at each particular time. Species may have multiple interaction partners, but only interact when opportunity arises. 
