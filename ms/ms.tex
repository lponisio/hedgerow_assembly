...
%% marilia:
%% I'm not totally sure what this adds to the methods. Since most
%% people reading this paper will not understand what "spectral
%% properties" are (I don't in fact =/ ) it is a bit of a misdirection
%% since we are not interested in it. I think it is enough to point to
%% your paper which is using the Alg Con the way we want to. 

%% removed:
% Algebraic connectivity is
% used to describe spectral properties of complex graphs other than
% ecological networks \citep[e.g.,][]{fiedler1973algebraic,
%   costa2007characterization, mohar1991laplacian, de2007old}.

% what's the mean diameter of a graph? Also can we use the
%% term network instead of graph to be mote consistent with the rest
%% of the methods? We use graph a bit in the change point section, but
%% I think it is explained a bit better there? 

%% I also removed the e.g.'s because your paper is all we want to
%% cite, and not "for example Gariarsa et al." 
...
during assembling, we next explored the species most likely
responsible for the observed shifts in interaciton patterns. Based on
a preferential attachment-like mechanism, we expect that the most
persistent and high degree species would remain stable in the network
core during assembly, and would thus contribute the least to the
changing points in network organization. Surprisingly however, we
encountered the opposite: the species that were most variable in their
network position and thus contributed most to network reorganizations
where species with the highest degrees (i.e., most generalized) and
...
%% marilia: not totally sure where you are going here. I was thinking
%% it could be expanded into a whole paragraph on the fact that we
%% don't really understand what effects species persistence, and how
%% that is problematic with understanding netwokr robustness. But
%% maybe it is a bit too far removed from the main message of the
%% paper? Maybe something could be added in the last papergraph about
%% needed more mechanistic models? 
 
%  However, the robustness of ecological
% communities to disturbances might depend on the identity of the
% species mostly being affected by the disturbance (REF). For example,
% networks might be more sensible to the disturbance of a generalist
% when compared to a specialist (REF).
