...
%DIF DEL ms.tex        Mon Aug 29 10:15:42 2016
%DIF ADD ms_matt.tex   Tue Aug 30 10:08:36 2016
...
\begin{abstract}  
  ...
  Central Valley of California. Species are highly dynamic in their
  \DIFdelbegin \DIFdel{positions in the networks}\DIFdelend \DIFaddbegin \DIFadd{network position}\DIFaddend , causing assembly to be punctuated by
  significant reorganizations of interactions. Non-assembling mature
  hedgerows and weedy field margins did not undergo as many
  significant network reorganizations, suggesting environmental
  factors alone cannot account for the changes observed in assembling
  communities. Pollinator species were opportunistic in the flowers
  they visited\DIFdelbegin \DIFdel{, and in fact }\DIFdelend \DIFaddbegin \DIFadd{. Indeed, }\DIFaddend the most persistent and generalized
  species were \DIFaddbegin \DIFadd{also the }\DIFaddend most variable in their network positions --- the
  opposite of \DIFdelbegin \DIFdel{what }\DIFdelend \DIFaddbegin \DIFadd{which }\DIFaddend would be expected by preferential attachment. High
  ...
  extinctions. The resilience of networks to cascading perturbations,
  however, increased as the communities assembled, at least partially
  due to accumulating species richness. Our results elucidate some of
  the mechanisms underlying plant-pollinator network assembly and
  restoration, while providing further evidence that hedgerows are a
  valuable tool for promoting species conservation and ecosystem
  provision in agricultural areas.
\end{abstract}
...
more temporally persistent \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Olesen2008}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\flagged{I don't think you need this one as already at start \citep{Olesen2008}}}\DIFaddend . In addition, using a
...
attach to the well-connected, generalist core. % In addition,
...
dataset of plant-pollinator communities \DIFdelbegin \DIFdel{assembling }\DIFdelend \DIFaddbegin \DIFadd{assembly }\DIFaddend following hedgerow
...
species \DIFaddbegin \DIFadd{that }\DIFaddend are most variable in their network position --- and thus
important contributors \DIFaddbegin \DIFadd{to }\DIFaddend network reorganizations --- are less persistent
and connected species. To further explore the mechanisms underlying
the temporal dynamics \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend the networks, we examine patterns in the
...
\DIFaddbegin \DIFadd{\flagged{Overall I think the introduction is a clear and logical progression of ideas and nicely explained. Good job guys! The only thing I might change is the restoration paragraph. I think you might even be better trying to merge with the hedgerows paragraph. The reason I say this is that the paragraph before the restoration one ends super well and focusses the reader, but I then got sidetracked when reading the restoration one because it goes down a different tangent - Marilia: maybe I got used to it already, but I don't agree with him...}
}\DIFaddend %% objectives
...
and persisting at the assembling hedgerows. \DIFdelbegin \DIFdel{Though some new plant
species may establish themselves in the hedgerows, because the plant
species }\DIFdelend \DIFaddbegin \DIFadd{Plant
species }\DIFaddend richness is based on the flowers in the network and not the
presence of a \DIFdelbegin \DIFdel{plant }\DIFdelend \DIFaddbegin \DIFadd{particular plant species }\DIFaddend at a site\DIFdelbegin \DIFdel{, the }\DIFdelend \DIFaddbegin \DIFadd{. Thus, though some new plant
species may establish themselves in the hedgerows, the }\DIFaddend increase in plant richness \DIFaddbegin \DIFadd{\flagged{wouldn't be pollinator here?}}\DIFaddend is likely due to plants attracting visitors\DIFaddbegin \DIFadd{, }\DIFaddend as they offer better rewards.\DIFaddbegin \DIFadd{\flagged{This sentence is hard to follow - Marilia: I tried to make it more clear}
}\DIFaddend 
...
\DIFaddbegin \DIFadd{\flagged{I think the results are nicely outlined, however I think you can get away with letters instead of the verbal description of statistics that are currently in the parentheses. Marilia: I agree...}
}
...
points. \DIFdelbegin \DIFdel{These network reorganizations are unlikely }\DIFdelend \DIFaddbegin \DIFadd{If these network reorganizations were }\DIFaddend a product of
environmental forces alone\DIFdelbegin \DIFdel{because the }\DIFdelend \DIFaddbegin \DIFadd{, we would explect to observe the same changing points at the same periods, consistently across all sites. However, }\DIFaddend network changing points in
non-assembling communities are less frequent, \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{\flagged{not sure I understand this. Marilia - I think it is well exaplained but I tried to change it a little} and there are }\DIFaddend few consistent
...
different from what is expected by preferential attachment \DIFaddbegin \DIFadd{\flagged{the impression I got from the introduction was that preferential attachment and changing points were mutually exclusive. Marilia - they are for the most part, but I think if there were LOTS of new specialists being added, we would see changig points, but not necessarily this wouldn't be preferential attachment. I think our thought process becames clear through the paragraph - the main point is that there is no stable core - so I wouldn''t change anything}}\DIFaddend . Based on a
...
network positions. \DIFaddbegin \DIFadd{\flagged{Nice result and interpretation!}
}\DIFaddend 
...
assembly \citep{valdovinos2013adaptive} \DIFaddbegin \fitted{There is a Bascompte, Nuismer and Jordano paper that might be applicable to cite here}\DIFaddend . In addition, the few
...
models of mutualistic \DIFdelbegin \DIFdel{system }\DIFdelend \DIFaddbegin \DIFadd{systems }\DIFaddend to generate testable predictions, along
...
\DIFadd{\flagged{Discussion of results is clear and perceptive. Only part I'm not completely convinced by is the stability side of things--just because I'm not sure that stability can be reliably thought to have increased because richness increases. But I don't have a better argument than that right now, so see what Paulo \& Claire say. Figures are all great, although I thought perhaps 5 \& 6 could be combined? Marilia - I don't agree, they're dealing with different stuff}
}\DIFaddend 
