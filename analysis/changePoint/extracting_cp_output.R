...
rm(list=ls())
library(reshape)
results<-read.table('~/Dropbox/hedgerow_assembly/analysis/changePoint/cptPeel/results_4.txt',
                  sep=' ')

logs<-read.table('~/Dropbox/hedgerow_assembly/analysis/changePoint/cptPeel/LogLs_4.txt',
                 sep=' ')
...
#checking if there is more than 1 value, and if yes, returns the index with 
#the maxlog
for(row in 1: dim(results)[1]){
  if(results$numberOfOnes[row]>1){#se eh true tem mais de um ai tem que ver o log 
