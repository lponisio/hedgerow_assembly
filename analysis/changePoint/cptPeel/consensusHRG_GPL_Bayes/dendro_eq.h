...
cnode::cnode()  { index = -1; degree = 0; parent = -1; weight = 0.0; children = NULL; lastChild = NULL; }
cnode::~cnode() {
	child *curr, *prev;
	curr = children;
	while (curr != NULL) {
...
public:
	...
	split(); ~split();					// 
	void initializeSplit(const int);		// initialize split string
	bool	checkSplit();					// guarantee split is well-formed
};
split::split() { s = ""; }
split::~split() {}
void split::initializeSplit(const int n) { s = ""; for (int i=0; i<n; i++) { s += "-"; } }
bool split::checkSplit() { if (s == "" or s.find("-",0) != string::npos) { return false; } else { return true; } }
...
public:
	...
	void			addConsensusSplit(string,double,double);	// add a split to splithist
...
dendro::~dendro() {
	...
	if (gs	    != NULL) { for (int i=0; i<ng; i++) {delete gs[i];} delete [] gs;           gs = NULL; }    
...
void dendro::buildDendrogram() {
	...
	if (gs == NULL) { cout << "Error: cannot build dendrogram without a graph structure.\n"; return; }
 ...
 */
	...
	for (int i=0; i<(n-1); i++) {
		...
		ei    = internal[i].e;
		internal[i].p = (double)(ei+1) / (double)(nL_nR * ng+2);
		//~ internal[i].p = (double)(ei) / (double)(nL_nR * ng);
		//~ if (ei == 0 or ei == nL_nR * ng) { dL = 0.0; }
		//~ else                        { dL = ei * log(internal[i].p) + (nL_nR * ng - ei) * log(1.0-internal[i].p); }
        dL = lgamma(ei+1) + lgamma(nL_nR * ng - ei + 1) - lgamma(nL_nR * ng + 2);
		internal[i].logL = dL;
		L += dL;
	}
...
int dendro::computeEdgeCount(const int a, const short int atype, const int b, const short int btype) {
	...
	if (nA < nB) {
		...
		for (int i=0; i<nA; i++) {
			for (int j=0; j<ng; j++) {
				current = gs[j]->getNeighborList(treeList[i]);
...
void dendro::addConsensusSplit(string splitKey, double splitValue, double numConsensusTrees) {
	double     oldSplitvalue; 
	...
	// First we obtain the current key value pair (returns 0.0 if not a valid key) and then remove this from
	// splithist.  We update the value and reinsert the new key-value pair into splithist.
	oldSplitvalue = splithist->returnValue(splitKey);
	splithist->deleteItem(splitKey);
	splithist->insertItem(splitKey, oldSplitvalue + splitValue / numConsensusTrees);
...
bool dendro::importDendrogramStructure(const string in_file) {
	string bracketL, bracketR, sL, sR, sLtype, sRtype, sp, se, sn, ngtxt;
	...
	fin >> ngtxt >> ng;			// read in number of G's
	...
	for (int i=0; i<(n-1); i++) {
		...
		ei    = internal[i].e;
		//~ if (ei == 0 or ei == nL_nR * ng) { dL = 0.0; }
		//~ else                        { dL = (double)(ei) * log(internal[i].p) + (double)(nL_nR * ng - ei) * log(1.0-internal[i].p); }
        dL = lgamma(ei+1) + lgamma(nL_nR * ng - ei + 1) - lgamma(nL_nR * ng + 2);
		internal[i].logL = dL;
		L += dL;
	}
...
bool dendro::monteCarloMove(double& delta, bool& ftaken, const double T) {
	...
	if (t == LEFT) {								// 
		if (mtr.randExc() < 0.5) {					// ## LEFT ALPHA move: ((i,j),k) -> ((i,k),j)
			...
			p_y  = (double)(e_y+1) / (double)(n_y * ng+2);
			//~ p_y  = (double)(e_y) / (double)(n_y * ng);
			//~ if (e_y == 0 or e_y == n_y * ng)   { L_y = 0.0; }
			//~ else						{ L_y = (double)(e_y) * log(p_y) + (double)(n_y * ng - e_y) * log(1.0-p_y); }
            L_y = lgamma(e_y+1) + lgamma(n_y * ng - e_y + 1) - lgamma(n_y * ng + 2);

			n_x  = (n_i+n_k)*n_j;
			e_x  = internal[x].e + internal[y].e - e_y;						// e_yj
			//~ p_x  = (double)(e_x) / (double)(n_x * ng);
			p_x  = (double)(e_x+1) / (double)(n_x * ng+2);
			//~ if (e_x == 0 or e_x == n_x * ng)	{ L_x = 0.0; }
			//~ else						{ L_x = (double)(e_x) * log(p_x) + (double)(n_x * ng - e_x) * log(1.0-p_x); }
            L_x = lgamma(e_x+1) + lgamma(n_x * ng - e_x + 1) - lgamma(n_x * ng + 2);
...
//		if (curr != NULL) { cout << ">> M[" << i << "] <<" << endl; }
	...
	for (int i=0; i<ii; i++) {
		...
		while (sit != NULL) {
			if (sit->type == GRAPH) { fout << gs[0]->getName(sit->index) << " (G) "; }
