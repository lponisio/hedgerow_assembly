...
void dendro::buildDendrogram() {
	//~ if (g == NULL and gs == NULL) { cout << "Error: cannot build dendrogram without a graph structure.\n"; return; }
 ...
 */
	...
	// --- UPDATE: We now also overcount by a factor of ng, but we do not correct at this time 
	// in order to keep integer values.
	...
	// UPDATE: Now we need to divide e_i by ng
    
    // UPDATE: now using Beta(1,1) priors on the parameter p
	...
	for (int i=0; i<(n-1); i++) {
		...
		ei    = internal[i].e;
		internal[i].p = (double)(ei+1) / (double)(nL_nR * ng + 2);      // this is an expected value of  p
...
int dendro::computeEdgeCount(const int a, const short int atype, const int b, const short int btype) {
	...
	//UPDATE: Count is now inflated by a factor of ng.
