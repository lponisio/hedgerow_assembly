...
struct ioparameters {
	...
	bool			flag_name;		// flag including input name file
...
int main(int argc, char * argv[]) {
	ioparm.n		= 0;					// 
	ioparm.timer   = 20;				// 
	ioparm.flag_make = ioparm.flag_f = ioparm.flag_name = ioparm.flag_m = false;
	...
	if (parseCommandLine(argc, argv)) {
		...
		if (ioparm.flag_f or ioparm.flag_m) {
...
bool parseCommandLine(int argc, char * argv[]) {
	...
	if (argc==1) {
		...
		while (argct < argc) {
			...
			} else if (temp == "-f" and !ioparm.flag_make and !ioparm.flag_m) {
...
//				ioparm.f_stat    = ioparm.d_dir + ioparm.s_scratch + "-L.xy";
				safeExit         = true;
			} else if (temp == "-m" and !ioparm.flag_make  and !ioparm.flag_f) {
				ioparm.flag_m = true;				// -f is mutually exclusive with -make
				argct++;
...
//				ioparm.f_stat    = ioparm.d_dir + ioparm.s_scratch + "-L.xy";
	...
	if (ioparm.flag_f or ioparm.flag_m)         { ioparm.f_stat     = ioparm.d_dir + ioparm.s_scratch + "-L.xy";  }
...
bool readInputFile() {
	...
	if (ioparm.flag_f) { ioparm.ng = 1;}
	d->ng = ioparm.ng;
	d->gs = new graph *[ioparm.ng];
	
	for (int j=0; j<ioparm.ng; j++) {
		d->gs[j] = new graph (n);						// make new graph with n vertices
		...
		cout << "   edges: [0]"<<endl;
		if (ioparm.flag_f) {in_file = ioparm.f_in;} else {in_file = ioparm.f_ins.at(j);}
