          ...
          STRUCTMODEL *sm, int alg_type)
{
  ...
  slacks[cset.lhs[j]->slackid]=MAX(slacks[cset.lhs[j]->slackid],
    ...
    //~ for(i=0; i<cset.m; i++)  {
      //~ printf("slack %ld   ", cset.lhs[i]->slackid);
      //~ printf("docnum %ld  ::", cset.lhs[i]->docnum);
      //~ printf("orgdocnum %ld  ::", cset.lhs[i]->orgDocNum);
    //~ }
  ...
  for (i=1; i<sm->svm_model->sv_num; i++ ) {
    docnum = sm->svm_model->supvec[i]->docnum;
    sm->svm_model->supvec[i]->orgDocNum = cset.lhs[docnum]->orgDocNum;
    k = sm->svm_model->supvec[i]->orgDocNum;
    k = 0;
  }
  char mufile[200];
  strcpy(mufile,lparm->modelfile);
  strcat(mufile,"_mu.dat");
  FILE *mufl = fopen(mufile,"w");
  for (k=1; k<sm->svm_model->sv_num; k++ ) {
		fprintf(mufl,"%ld ",sm->svm_model->supvec[k]->docnum);
		fprintf(mufl,"%ld ",sm->svm_model->supvec[k]->orgDocNum);
		fprintf(mufl,"%.8g\n",sm->svm_model->alpha[k]);
      }
  fclose(mufl);
  
  char wfile[200];
  strcpy(wfile,lparm->modelfile);
  strcat(wfile,"_w.dat");
  FILE *wfl = fopen(wfile,"w");
  //~ FILE *wfl = fopen("w.dat","w");
  for (k=0; k<sizePsi; k++ ) {
		fprintf(wfl,"%.8g ",sm->w[k]);
      }
  fclose(wfl);
