plot.panel <- function(dats,
    ...
    plotting.loop <- function(){
        ys <- aggregate(list(y=dats[,y1]),
                        ...
                        mean, na.rm=TRUE)
        if(plotPoints){
            names(pchs) <- unique(ys[, agg.col])
            names(col.points) <- unique(ys[, agg.col])
            points(x=ys$x,
                   y=ys$y,
                   pch=pchs[as.character(ys[, agg.col])],
                   col=col.points[as.character(ys[, agg.col])],
                   cex=1.6)
            points(x=ys$x,
                   y=ys$y,
                   pch=1,
